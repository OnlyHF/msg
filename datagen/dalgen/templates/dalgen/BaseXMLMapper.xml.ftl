<@pp.dropOutputFile />
<#if gen.xmlMapper?? && gen.dbType=='mysql'>
<#assign xmlMapper =gen.xmlMapper>
<#assign table  = gen.table>
<#assign doMapper =gen.doMapper>
<@pp.changeOutputFile name = "/src/main/resources/${xmlMapper.xmlPath}/${xmlMapper.doMapper.baseClassName}.xml" />
   ...
   <update id="updateBy${fields.name?cap_first}" parameterType="${xmlMapper.doObject.packageName}.${xmlMapper.doObject.className}">
       <#assign c_idx = 0>
        update ${table.tableName} set
            <#list table.columnList as column>
                  <#if column.autoIncrement?string('yes', 'no')=='no'  && column.javaName !=fields.name && column.javaName !='gmtCreate' && column.javaName !='createTime' && column.javaName !='createUser' && column.javaName !='createUsername' && column.uniqueIndex?string('yes','no')=='no' >
                         <#if c_idx gt 0 >
...
<#if doMapper.uniqueIndexFields?? >
  <#list doMapper.uniqueIndexFields.fieldsList as fields>
   <update id="updateByUkIndex${fields.name?cap_first}" parameterType="${xmlMapper.doObject.packageName}.${xmlMapper.doObject.className}">
       <#assign c_idx = 0>
        update ${table.tableName} set
            <#list table.columnList as column>
                  <#if column.autoIncrement?string('yes', 'no')=='no'  && column.javaName !=fields.name && column.javaName !='gmtCreate' && column.javaName !='createTime' && column.javaName !='createUser' && column.javaName !='createUsername' &&  column.uniqueIndex?string('yes','no')=='no' >
    ...
    <delete id="deleteByUkIndex${fields.name?cap_first}" parameterType="${fields.javaType}">
          delete from ${table.tableName} where ${fields.sqlName} = ${"#"}{${fields.name},jdbcType=${fields.sqlType}}
    </delete>

    <select id="getByUkIndex${fields.name?cap_first}" resultMap="BaseResultMap" parameterType="${fields.javaType}">
        select <include refid="Base_Column_List"/>  from  ${table.tableName}  where ${fields.sqlName} = ${"#"}{${fields.name},jdbcType=${fields.sqlType}}
    </select>
  </#list>
</#if>
